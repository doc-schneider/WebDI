...
class BoxViewer(Viewer):
    def __init__(self, View):
        super().__init__(View.document_pathtype, View.encode_type)

    # photoTimeline specifics: Time, photo information
    def init_photoTimeline(self, documenttable, time_interval, use_thumbnail):
        self.use_thumbnail = use_thumbnail
        self.update_photoTimeline(documenttable, time_interval)

    def update_photoTimeline(self, documenttable, time_interval):
        self.timeinterval = pd.Interval(time_interval['TIME_FROM'].iloc[0],
                                   time_interval['TIME_TO'].iloc[0], closed='left')
    ...
    def update(self, documenttable):
        ...
        if self.index_groups:
            self.boxShow = documenttable.data.loc[documenttable.data['GROUP_INDEX']==self.index_show, ].copy()
        else:
    ...
    def encode_data(self):
        return self.boxShow['LOCATION_DOCUMENT'].apply(encode_data, args=(self.encode_type,)).tolist()
